...
android_pipe_find_type(const char*  pipeName)
{
    PipeServices* list = _pipeServices;
    int           count = list->count;
    int           nn;
...
pingPongPipe_sendBuffers( void* opaque, const AndroidPipeBuffer* buffers, int numBuffers )
{
    ...
    /* Wake up any waiting writers if we consumed something */
    if (pipe->count > 0 && (pipe->flags & PIPE_WAKE_WRITE)) {
...
pingPongPipe_recvBuffers( void* opaque, AndroidPipeBuffer* buffers, int numBuffers )
{
    ...
    /* Wake up any waiting readers if we produced something */
    if (pipe->count < PINGPONG_SIZE && (pipe->flags & PIPE_WAKE_READ)) {
