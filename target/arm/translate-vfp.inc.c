 ...
 * whether VFP is enabled: this should be true for FMXR/FMRX accesses
 * to FPSID, FPEXC, MVFR0, MVFR1, MVFR2, and false for all other insns.
 */
static bool full_vfp_access_check(DisasContext *s, bool ignore_vfp_enabled)
{
    if (s->fp_excp_el || (!s->vfp_enabled && !ignore_vfp_enabled)) {
...
static bool trans_VMOV_to_gp(DisasContext *s, arg_VMOV_to_gp *a)
{
    ...
    if (a->opc1 & 2) {
        ...
        size = 1;
        offset = (a->opc2 & 2) ? 16 : 0;
    } else if (a->u == 0 && (a->opc2 & 2) == 0) {
        size = 2;
        offset = 0;
    } else {
        return false;
    ...
    if (size != 2 && !arm_dc_feature(s, ARM_FEATURE_NEON)) {
        return false;
    }
...
static bool trans_VMOV_single(DisasContext *s, arg_VMOV_single *a)
{
    ...
    if (a->l) {
        ...
        tmp = tcg_temp_new_i32();
        tcg_gen_ld_f32(tmp, cpu_env, vfp_reg_offset(false, a->vn));
        if (a->rt == 15) {
            /* Set the 4 flag bits in the CPSR.  */
            gen_set_nzcv(tmp);
            tcg_temp_free_i32(tmp);
        ...
        tmp = load_reg(s, a->rt);
        tcg_gen_st_f32(tmp, cpu_env, vfp_reg_offset(false, a->vn));
        tcg_temp_free_i32(tmp);
    }
...
static bool trans_VMOV_64_sp(DisasContext *s, arg_VMOV_64_sp *a)
{
    ...
    if (a->op) {
        ...
        store_reg(s, a->rt, tmp);
        tmp = tcg_temp_new_i32();
        tcg_gen_ld_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm + 1));
        store_reg(s, a->rt2, tmp);
    } else {
        /* gpreg to fpreg */
        tmp = load_reg(s, a->rt);
        tcg_gen_st_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm));
        tmp = load_reg(s, a->rt2);
        tcg_gen_st_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm + 1));
    }
...
static bool trans_VMOV_64_dp(DisasContext *s, arg_VMOV_64_sp *a)
{
    ...
    if (a->op) {
        /* fpreg to gpreg */
        tmp = tcg_temp_new_i32();
        tcg_gen_ld_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm * 2));
        store_reg(s, a->rt, tmp);
        tmp = tcg_temp_new_i32();
        tcg_gen_ld_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm * 2 + 1));
        store_reg(s, a->rt2, tmp);
    } else {
        /* gpreg to fpreg */
        tmp = load_reg(s, a->rt);
        tcg_gen_st_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm * 2));
        tcg_temp_free_i32(tmp);
        tmp = load_reg(s, a->rt2);
        tcg_gen_st_f32(tmp, cpu_env, vfp_reg_offset(false, a->vm * 2 + 1));
        tcg_temp_free_i32(tmp);
    }
...
static bool trans_VRINTR_dp(DisasContext *s, arg_VRINTR_sp *a)
{
    ...
    tmp = tcg_temp_new_i64();
    tcg_gen_ld_f64(tmp, cpu_env, vfp_reg_offset(true, a->vm));
    fpst = get_fpstatus_ptr(false);
