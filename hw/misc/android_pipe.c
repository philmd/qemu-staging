...
#include "exec/ram_addr.h"      /* AWOOGA! */
...
/* from AOSP version include/hw/android/goldfish/device.h, most likely
 * needs to use proper qemu abstractions */
...
/* hardcod for now */
static inline gboolean goldfish_guest_is_64bit(void) {
    return TRUE;
}
...
typedef struct Pipe {
    ...
    PipeDevice*                device;
    uint64_t                   channel; /* opaque handle */
    void*                      opaque;
    ...
    if (state != 0) {
        ...
        if (service == NULL) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: No QEMU pipe service named '%s'", __func_, name);
            g_free(name);
            return NULL;
        }
    }
...
struct PipeDevice {
    /* TODO: use the dtb sysbus stuff instead - like lionhead */
/*     struct goldfish_device dev; */
    AndroidPipeState *ps;       /* HACK: backlink to instance state */
...
static void android_pipe_realize(DeviceState *dev, Error **errp)
{
    ...
    /* TODO: so what device mechanism are we going to use? */
#if 0
    ...
    /* For now disable VM save/restore */
#if 0
